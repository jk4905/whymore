...
class RobotMessagesController extends Controller
{

    const HEADER = '机器人消息';

    public static $status = ['1' => '启用', '2' => '禁止'];
    public static $type = ['1' => '用户发送', '2' => '帖子发送'];
...
//        });
    ...
    {
        return Admin::grid(RobotMessage::class, function (Grid $grid) {
//            禁用行选择checkbox
...
//  `from` bigint(20) unsigned NOT NULL DEFAULT '0' COMMENT '用户QQ',
//  `to` bigint(20) unsigned NOT NULL COMMENT '机器人QQ',
//  `send_time` datetime NOT NULL COMMENT '发送时间',
//  `content` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '消息内容',
//  `images` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '消息内容中的图片链接',
//  `type` tinyint(3) unsigned NOT NULL COMMENT '消息类型，1-用户发送，2-帖子发送',
//  `status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '状态：1-有效，2-删除',
//  `group` bigint(20) unsigned NOT NULL DEFAULT '0' COMMENT '群号',
            $grid->id('ID')->sortable();

            $grid->from('发送者');
            $grid->to('机器人QQ');
            $grid->group('群号');
            $grid->send_time('发送时间')->sortable();
            $grid->content('消息内容');
            $grid->images('图片')->image('', 40, 40);
            $grid->type('消息类型')->using(self::$type);
            $grid->status('状态')->using(self::$status);
